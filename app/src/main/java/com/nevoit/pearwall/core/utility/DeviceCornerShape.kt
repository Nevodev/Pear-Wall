package com.nevoit.pearwall.core.utility
...
fun deviceCornerShape(
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S) return UnevenRoundedRectangle(
        12.dp,
        12.dp,
        12.dp,
        12.dp
...
fun deviceCornerShape(
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S) {
        fun resolveCorner(hasCorner: Boolean): Dp {
            return if (hasCorner) 12.dp else 0.dp
        }

        return UnevenRoundedRectangle(
            resolveCorner(topLeft),
            resolveCorner(topRight),
            resolveCorner(bottomLeft),
            resolveCorner(bottomRight)
        )
    }
    return remember(view, configuration, padding, topLeft, topRight, bottomRight, bottomLeft) {
        fun getCornerRadius(position: Int, enabled: Boolean): Dp {
